...
void UMixedRealityRenderer::BeginPlay()
{
  ...
  // Check for the existence of external camera config
  ExternalCameraConfig =
    GetWorld()->SpawnActor<AExternalCameraConfig>(spawnInfo);
  ...
  ExternalCamera->Hand_DEPRECATED = EControllerHand::ExternalCamera;
  ExternalCamera->MotionSource = FName("ExternalCamera");
  // TODO: I don't like this part, but it seems to be the only way
  // to make it work reliably.
  //ExternalCamera->SetTrackingSource(ControllerType);
...
)
{
  ...
  if (ExternalCameraConfig == nullptr)
    return;
...
#if ENGINE_MAJOR_VERSION == 4 && ENGINE_MINOR_VERSION >= 17
	  ISpectatorScreenController* spectatorScreenController = HMDDevice->GetSpectatorScreenController();
	  if (spectatorScreenController != nullptr)
	  {
		  spectatorScreenController->SetSpectatorScreenMode(ESpectatorScreenMode::Distorted);
	  }
#endif
    }
  }
